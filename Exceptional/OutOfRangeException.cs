...
namespace Exceptional
{
    ...
    public class OutOfRangeException : Exception
    {
        ...
        public OutOfRangeException(int pageNumber, int totalPages)
        {
            ...
            if(pageNumber < 0)
            {
                _message.Append("Invalid page number. Page number can only be a positive integer number.");
            }
            else if(pageNumber > totalPages)
            {
                _message.Append($"Page requested is out of range. Collection results has a maximum of {totalPages} pages to query.\nUse a value that is less than or equal to {totalPages}");
            }
        }
        /// <summary>
